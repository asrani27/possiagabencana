...
<div class="row">
  <div class="col-lg-12">
    <div class="card card-primary card-outline">
      ...
      <form method="post" action="/admin/rekapitulasi_luar/edit/{{$data->id}}">
        @csrf
      <div class="card-body">
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <select name="kelurahan_id" class="form-control select2" required>
                ...
                <option value="{{$item->id}}" {{$item->id == $data->kelurahan_id ? 'selected':''}}>{{$item->nama}} - KEC. {{$item->kecamatan->nama}}</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-1">
              <input type="text" class="form-control" name="jumlah" value="{{$data->jumlah}}" required maxlength="6" onkeypress="return hanyaAngka(event)">
            </div>
          </div>
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-1">
              <input type="text" class="form-control" name="perempuan" style="text-transform: uppercase" value="{{$data->perempuan}}" required maxlength="6" onkeypress="return hanyaAngka(event)">
            </div>
          </div>
          <div class="form-group row">
            <label for="inputPassword3" class="col-sm-2 col-form-label">Jumlah Laki-laki</label>
            <div class="col-sm-1">
              <input type="text" class="form-control" name="laki" style="text-transform: uppercase" value="{{$data->laki}}" required maxlength="6" onkeypress="return hanyaAngka(event)">
            </div>
          </div>
          <div class="form-group row">
